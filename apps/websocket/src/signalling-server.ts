...
async function startSubscriberChannel() {
  await subscribeToChannel();
}

startSubscriberChannel().catch((err) => {
  ...
  ws.on("message", async (message: Buffer | ArrayBuffer) => {
    ...
    switch (data.type) {
      ...
      case EventTypes.CREATE_WEBRTC_TRANSPORT:
        const createWebrtcPayload =
          data.message as EventPayloadMap[typeof EventTypes.CREATE_WEBRTC_TRANSPORT];
        const transportRoom = roomManager.getRoom(createWebrtcPayload.roomId);
        if (transportRoom) {
          await transportRoom.createWebRTCTransport(
            createWebrtcPayload,
            ws,
            peerId
          );
      ...
      case EventTypes.CONNECT_CONSUMER_TRANSPORT:
        const producerConsumerConnectPayload =
          data.message as EventPayloadMap[typeof EventTypes.CONNECT_CONSUMER_TRANSPORT];
        const connectRoom = roomManager.getRoom(
          producerConsumerConnectPayload.roomId
        );
        if (connectRoom) {
          await connectRoom.producerAndConsumerConnectTotransport(
            producerConsumerConnectPayload,
            peerId
          );
        }
        break;
      ...
      case EventTypes.PRODUCE:
        const producePayload =
          data.message as EventPayloadMap[typeof EventTypes.PRODUCE];
      ...
      case EventTypes.CONSUME:
        const consumePayload =
          data.message as EventPayloadMap[typeof EventTypes.CONSUME];
      ...
      case EventTypes.RESUME_CONSUMER:
        const resumeConsumerPayload =
          data.message as EventPayloadMap[typeof EventTypes.RESUME_CONSUMER];
        const resumeRoom = roomManager.getRoom(resumeConsumerPayload.roomId);
        if (resumeRoom) {
          await resumeRoom.handleResumeConsumer(
      ...
      case EventTypes.PRODUCER_CLOSED:
        const producerClosedPayload =
          data.message as EventPayloadMap[typeof EventTypes.PRODUCER_CLOSED];
        const closeRoom = roomManager.getRoom(producerClosedPayload.roomId);
        if (closeRoom) {
          await closeRoom.handleProducerClosed(
      ...
      case EventTypes.LOCAL_USER_MEDIA_TOGGLED:
        const localUserVideoOff =
          data.message as EventPayloadMap[typeof EventTypes.LOCAL_USER_MEDIA_TOGGLED];
        const userRoom = roomManager.getRoom(localUserVideoOff.roomId);
        publishMessage("room:" + userRoom, {
          type: EventTypes.LOCAL_USER_MEDIA_TOGGLED,
          message: localUserVideoOff,
        });
        break;
      case EventTypes.SEND_CHAT_MESSAGE:
        const chatMessagePayload =
          data.message as EventPayloadMap[typeof EventTypes.SEND_CHAT_MESSAGE];
        publishMessage(`room:${chatMessagePayload.roomId}`, {
      ...
      case EventTypes.TYPING:
        const typingPayload =
          data.message as EventPayloadMap[typeof EventTypes.TYPING];
